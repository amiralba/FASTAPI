...
@app.get("/posts/{id}")
def get_post(id: int, response: Response):  #hatman byd id: int bezarim ke id o be int tabdil kone ke def find_post doros kar kone
    post= find_post(id)
    if not post:
...
def delete_posts(id: int):
    ...
    if index == None: #ino mizarim ke age id besh bedim ke mojod nabashe betone ye javabi bede va error nade
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"post with id:{id} was not found")
    my_posts.pop(index)
